    ...
    <aside :class="['sidebar-nav', sidebarOpen ? 'open' : '']">
      <div class="sn-brand">
        <div class="sn-brand-icon" style="background: transparent;">
          <img src="../../assets/logo-capacitaciones.png" alt="Logo"
      ...
      <nav class="sn-links">
        <RouterLink to="/usuario/dashboard" @click="sidebarOpen = false">
          <svg class="nav-icon" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"
            viewBox="0 0 24 24">
            <path
        ...
        <RouterLink to="/usuario/mensajes" @click="sidebarOpen = false">
          <svg class="nav-icon" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"
            viewBox="0 0 24 24">
            <path
        ...
        <RouterLink to="/usuario/licencias" @click="sidebarOpen = false">
          <svg class="nav-icon" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"
            viewBox="0 0 24 24">
            <path
              d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
          </svg>
          Mis licencias (Empresas)
        </RouterLink>
      ...
      <header class="topbar">
        ...
        <div class="topbar-left">
          <template v-for="(crumb, idx) in breadcrumbs" :key="crumb.path">
            ...
            <RouterLink v-if="idx < breadcrumbs.length - 1" :to="crumb.path" class="breadcrumb-link">{{ crumb.name }}
            </RouterLink>
        ...
        <div class="topbar-right">
          ...
          <button class="icon-btn" @click="toggleTheme"
            ...
            <svg v-if="!isDark" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"
              viewBox="0 0 24 24">
          ...
          <div class="topbar-user" @click.stop="profileOpen = !profileOpen">
            <div class="topbar-avatar">
              <img :src="getAvatarUrl(auth.user?.avatar_url, auth.user?.id || auth.user?.name)" class="avatar-img"
                alt="avatar" />
            </div>
            <span class="topbar-name">{{ (auth.user?.name || '').slice(0, 20) }}</span>
            <svg class="topbar-chevron" :class="{ open: profileOpen }" fill="none" stroke="currentColor"
            ...
            <Transition name="slide-down">
              <div v-if="profileOpen" class="profile-dropdown" @click.stop>
                ...
                <RouterLink to="/usuario/perfil" class="pd-item" @click="profileOpen = false">
                  <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                  Mi perfil
