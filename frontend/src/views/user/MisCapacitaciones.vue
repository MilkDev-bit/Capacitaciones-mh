...
const typeIcon: Record<string, string> = { video: 'ðŸŽ¥', document: 'ðŸ“Ž', text: 'ðŸ“' }
      ...
      <div v-if="capacitaciones.length" class="courses-grid">
        ...
        >
          <div :class="['course-thumb', thumbClass[c.type] || 'thumb-default']">
            <span class="thumb-icon">{{ typeIcon[c.type] || 'ðŸ“š' }}</span>
          </div>
          <div class="course-body">
            <span class="course-type-badge">{{ typeLabel[c.type] || c.type }}</span>
            <h3 class="course-title">{{ c.title }}</h3>
            <p class="course-desc">{{ c.description || 'Sin descripciÃ³n' }}</p>
            <div class="course-cta">Continuar aprendiendo â†’</div>
          </div>
        </div>
      </div>
      <div v-else class="empty-state">
        <div class="empty-icon">ðŸ“š</div>
        <h3>No tienes cursos asignados aÃºn</h3>
        <p>Explora los cursos disponibles o pide a tu instructor un cÃ³digo de acceso.</p>
    ...
    <div v-if="activeTab === 'explorar'">
      ...
      <div class="code-banner">
        <div class="code-banner-left">
          <div class="code-key-icon">ðŸ”‘</div>
          <div>
            <strong>Â¿Tienes un cÃ³digo de acceso?</strong>
            <p>Ingresa el cÃ³digo de tu instructor para unirte a un curso privado.</p>
          </div>
        </div>
      ...
      <div v-if="cursosPublicos.length" class="courses-grid">
        <div v-for="c in cursosPublicos" :key="c.id" class="course-card public-course">
          <div :class="['course-thumb', thumbClass[c.type] || 'thumb-default']">
            <span class="thumb-icon">{{ typeIcon[c.type] || 'ðŸ“š' }}</span>
            <span v-if="c.inscrito" class="enrolled-ribbon">âœ“ Inscrito</span>
          </div>
          <div class="course-body">
            <span class="course-type-badge">{{ typeLabel[c.type] || c.type }}</span>
            <h3 class="course-title">{{ c.title }}</h3>
            ...
            <div class="course-footer-row">
              <span v-if="c.inscrito" class="badge badge-green">âœ“ Ya inscrito</span>
              <button
                v-else
              ...
              >
                {{ inscribiendose === c.id ? 'Inscribiendoâ€¦' : '+ Inscribirse gratis' }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div v-else class="empty-state">
        ...
        <h3>No hay cursos pÃºblicos disponibles</h3>
        <p>Pide a tu instructor que comparta el enlace o cÃ³digo de su curso.</p>
      </div>
    </div>
  </div>
</template>
