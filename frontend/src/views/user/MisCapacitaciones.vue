...
async function unirseConCodigo() {
  ...
  if (!code) { codigoError.value = 'Ingresa un cÃÂÃÂÃÂÃÂ³digo'; return }
  codigoError.value = ''; codigoSuccess.value = ''
  codigoLoading.value = true
  try {
    const res = await api.post('/unirse-con-codigo', { codigo: code })
    codigoSuccess.value = `ÃÂÃ¢ÂÂÃÂÃÂ¡Te uniste a "${res.data.title}"!`
    codigoInput.value = ''
    await loadMis()
    setTimeout(() => { codigoSuccess.value = ''; activeTab.value = 'mis' }, 2000)
  } catch (e: any) {
    codigoError.value = e.response?.data?.error || 'CÃÂÃÂÃÂÃÂ³digo invÃÂÃÂÃÂÃÂ¡lido'
...
const typeIcon: Record<string, string> = { video: 'ÃÂÃÂ°ÃÂÃÂ¸ÃÂÃÂ½ÃÂÃÂ¥', document: 'ÃÂÃÂ°ÃÂÃÂ¸ÃÂ¢Ã¢ÂÂ¬ÃÂÃÂÃÂ½', text: 'ÃÂÃÂ°ÃÂÃÂ¸ÃÂ¢Ã¢ÂÂ¬ÃÂÃÂÃÂ' }
      ...
      <div v-if="capacitaciones.length" class="courses-grid">
        ...
        >
          <div :class="['course-thumb', thumbClass[c.type] || 'thumb-default']">
            <span class="thumb-icon">{{ typeIcon[c.type] || 'ÃÂÃÂ°ÃÂÃÂ¸ÃÂ¢Ã¢ÂÂ¬ÃÂÃÂÃÂ¡' }}</span>
          </div>
          <div class="course-body">
            <span class="course-type-badge">{{ typeLabel[c.type] || c.type }}</span>
            <h3 class="course-title">{{ c.title }}</h3>
            <p class="course-desc">{{ c.description || 'Sin descripciÃÂÃÂÃÂÃÂ³n' }}</p>
            <div class="course-cta">Continuar aprendiendo ÃÂÃÂ¢ÃÂ¢Ã¢ÂÂ¬ÃÂ ÃÂ¢Ã¢ÂÂ¬Ã¢ÂÂ¢</div>
          </div>
        </div>
      </div>
      <div v-else class="empty-state">
        <div class="empty-icon">ÃÂÃÂ°ÃÂÃÂ¸ÃÂ¢Ã¢ÂÂ¬ÃÂÃÂÃÂ¡</div>
        <h3>No tienes cursos asignados aÃÂÃÂÃÂÃÂºn</h3>
        <p>Explora los cursos disponibles o pide a tu instructor un cÃÂÃÂÃÂÃÂ³digo de acceso.</p>
    ...
    <div v-if="activeTab === 'explorar'">
      ...
      <div class="code-banner">
        <div class="code-banner-left">
          <div class="code-key-icon">ÃÂÃÂ°ÃÂÃÂ¸ÃÂ¢Ã¢ÂÂ¬ÃÂÃÂ¢Ã¢ÂÂ¬ÃÂ</div>
          <div>
            <strong>ÃÂÃ¢ÂÂÃÂÃÂ¿Tienes un cÃÂÃÂÃÂÃÂ³digo de acceso?</strong>
            <p>Ingresa el cÃÂÃÂÃÂÃÂ³digo de tu instructor para unirte a un curso privado.</p>
          </div>
        </div>
        <div class="code-banner-right">
          ...
          <button class="btn btn-primary" :disabled="codigoLoading" @click="unirseConCodigo">
            {{ codigoLoading ? 'ÃÂÃÂ¢ÃÂ¢Ã¢ÂÂÃÂ¬ÃÂÃÂ¦' : 'Unirme' }}
          </button>
        </div>
      </div>
      ...
      <div v-if="cursosPublicos.length" class="courses-grid">
        <div v-for="c in cursosPublicos" :key="c.id" class="course-card public-course">
          <div :class="['course-thumb', thumbClass[c.type] || 'thumb-default']">
            <span class="thumb-icon">{{ typeIcon[c.type] || 'ÃÂÃÂ°ÃÂÃÂ¸ÃÂ¢Ã¢ÂÂ¬ÃÂÃÂÃÂ¡' }}</span>
            <span v-if="c.inscrito" class="enrolled-ribbon">ÃÂÃÂ¢ÃÂÃ¢ÂÂÃÂ¢Ã¢ÂÂ¬ÃÂ Inscrito</span>
          </div>
          <div class="course-body">
            <span class="course-type-badge">{{ typeLabel[c.type] || c.type }}</span>
            <h3 class="course-title">{{ c.title }}</h3>
            ...
            <div class="course-footer-row">
              <span v-if="c.inscrito" class="badge badge-green">ÃÂÃÂ¢ÃÂÃ¢ÂÂÃÂ¢Ã¢ÂÂ¬ÃÂ Ya inscrito</span>
              <button
                v-else
              ...
              >
                {{ inscribiendose === c.id ? 'InscribiendoÃÂÃÂ¢ÃÂ¢Ã¢ÂÂÃÂ¬ÃÂÃÂ¦' : '+ Inscribirse gratis' }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div v-else class="empty-state">
        <div class="empty-icon">ÃÂÃÂ°ÃÂÃÂ¸ÃÂÃ¢ÂÂÃÂÃÂ</div>
        <h3>No hay cursos pÃÂÃÂÃÂÃÂºblicos disponibles</h3>
        <p>Pide a tu instructor que comparta el enlace o cÃÂÃÂÃÂÃÂ³digo de su curso.</p>
      </div>
    </div>
  </div>
</template>
