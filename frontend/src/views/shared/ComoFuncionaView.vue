...
const PASOS: Paso[] = [
  {
    ...
    cuerpo:
      'Suscríbete y abre todo el catálogo mientras el plan siga activo, o compra suelto el curso que necesitas y consérvalo para siempre. Las dos vías dan la misma constancia.',
  ...
  },
  {
    ...
    cuerpo:
      'El total que ves es el que pagas. Los importes se calculan en centavos enteros de principio a fin, así que nunca aparece un peso de más ni de menos al llegar a Stripe.',
  ...
  },
  {
    ...
    titulo: ['PAGA', 'SEGURO'],
    cuerpo:
      'El cobro ocurre en Stripe: ningún dato de tu tarjeta pasa por nuestros servidores. Al terminar vuelves a una confirmación con tus accesos ya listos, no a una pantalla en blanco.',
  ...
  },
  {
    ...
    cuerpo:
      'Compra licencias corporativas, captura los correos de tus colaboradores y cada quien recibe su acceso por correo. Si alguien deja la empresa, reasignas su lugar.',
...
<template>
  <div :class="['cf', reducido && 'cf--estatico']">
    ...
    <div class="cf__paisaje" aria-hidden="true">
      <svg viewBox="0 0 1440 900" preserveAspectRatio="xMidYMax slice">
        ...
        <g class="cf__estrellas" :style="{ opacity: estrellas }" fill="#fff">
          <circle cx="140" cy="120" r="1.6" /><circle cx="330" cy="72" r="1.1" />
          <circle cx="520" cy="160" r="1.8" /><circle cx="690" cy="60" r="1.2" />
          <circle cx="860" cy="140" r="1.5" /><circle cx="1030" cy="86" r="1.9" />
          <circle cx="1210" cy="150" r="1.2" /><circle cx="1360" cy="70" r="1.6" />
          <circle cx="240" cy="240" r="1.3" /><circle cx="600" cy="266" r="1.5" />
          <circle cx="960" cy="230" r="1.1" /><circle cx="1300" cy="270" r="1.7" />
          <circle cx="430" cy="330" r="1.2" /><circle cx="1120" cy="345" r="1.4" />
        </g>
        ...
        <g class="cf__nubes" fill="#fff" opacity="0.34">
          <path :style="estiloNube1" d="M180 250c14-16 38-12 56-8 16-8 36-12 52 0 12-16 40-18 54-2 10-8 26-6 32 4H180z" />
          <path :style="estiloNube2" d="M980 178c12-14 32-10 48-7 14-7 31-10 45 0 10-14 34-15 46-2 9-7 22-5 27 3H980z" />
        </g>

        <!-- Cinco planos de colina, del más lejano al más cercano -->
        <path :style="estiloColina(0)" :fill="css(paleta.colinas[0]!)" d="M0,560 C240,500 420,600 720,540 C1020,480 1220,580 1440,520 L1440,960 L0,960 Z" />
        <path :style="estiloColina(1)" :fill="css(paleta.colinas[1]!)" d="M0,640 C260,580 460,690 760,630 C1060,570 1240,670 1440,610 L1440,960 L0,960 Z" />
        <path :style="estiloColina(2)" :fill="css(paleta.colinas[2]!)" d="M0,712 C300,660 520,760 820,706 C1120,652 1290,730 1440,690 L1440,960 L0,960 Z" />
        <path :style="estiloColina(3)" :fill="css(paleta.colinas[3]!)" d="M0,782 C280,742 540,822 840,780 C1140,738 1300,796 1440,764 L1440,960 L0,960 Z" />
        <path :style="estiloColina(4)" :fill="css(paleta.colinas[4]!)" d="M0,846 C320,818 560,872 860,842 C1160,812 1320,858 1440,836 L1440,960 L0,960 Z" />
      </svg>
    </div>
    ...
    <div v-if="!reducido" class="cf__escena" aria-hidden="true">
      <div class="cubo" :style="estiloCubo">
        <div v-for="(p, i) in PASOS" :key="p.clave" class="cara" :data-cara="i">
          ...
          <svg class="cara__icono" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
            <path :d="p.icono" />
          </svg>
    ...
    <div class="hud" aria-hidden="true">
      ...
      <div class="hud__barra"><div class="hud__fill" :style="{ width: pct + '%' }" /></div>
    ...
    <nav class="strip" aria-label="Pasos">
      <button
        v-for="(p, i) in PASOS"
        :key="p.clave"
        :class="['strip__dot', indice === i && 'is-on']"
        :aria-label="`Paso ${p.num}: ${p.nombre}`"
        :aria-current="indice === i ? 'step' : undefined"
        @click="irAPaso(i)"
      />
    </nav>
...
.cf__paisaje svg { width: 100%; height: 100%; display: block; }
.cf__paisaje path,
.cf__nubes path { will-change: transform; }
.cf__estrellas { transition: opacity 0.2s linear; }
...
.cara__icono { width: clamp(28px, 7vw, 44px); height: auto; color: var(--tinta); opacity: 0.85; }
.cara__nombre {
...
.cara[data-cara='0'] { transform: rotateX(-90deg) translateZ(calc(var(--lado) / 2)); }
.cara[data-cara='1'] { transform: translateZ(calc(var(--lado) / 2)); }
.cara[data-cara='2'] { transform: rotateY(90deg) translateZ(calc(var(--lado) / 2)); }
.cara[data-cara='3'] { transform: rotateY(180deg) translateZ(calc(var(--lado) / 2)); }
.cara[data-cara='4'] { transform: rotateY(-90deg) translateZ(calc(var(--lado) / 2)); }
.cara[data-cara='5'] { transform: rotateX(90deg) translateZ(calc(var(--lado) / 2)); }
...
.cf__brand { background: none; border: 0; padding: 0; cursor: pointer; }
.cf__brand img { height: 34px; width: auto; display: block; }
.cf__hdr-acciones { display: flex; gap: 0.5rem; }
.cf__link {
...
.cf__link:hover { border-color: var(--acento); color: var(--acento); }
.cf__link--solido { background: var(--acento); border-color: var(--acento); color: #14122c; }
.cf__link--solido:hover { filter: brightness(1.08); color: #14122c; }

/* ── Secciones ─────────────────────────────────────────── */
.cf__scroll { position: relative; z-index: 10; }
.cf__sec {
  min-height: 100vh;
  display: flex;
  align-items: center;
...
.tarjeta__linea { width: 3rem; height: 1px; background: var(--acento); margin-bottom: 1.1rem; }
.tarjeta__tag {
  margin: 0 0 1rem;
  font-size: 0.62rem;
...
.tarjeta__ctas { display: flex; flex-wrap: wrap; gap: 0.6rem; margin-top: 1.6rem; }
.tarjeta--der .tarjeta__ctas { justify-content: flex-end; }
.cta {
...
.cta--fantasma { background: transparent; color: var(--acento); }
.cta--fantasma:hover { background: var(--acento); color: #14122c; }
...
.plano svg { width: 26px; height: 26px; color: var(--tinta); opacity: 0.85; }
.plano__num { font-size: 1.5rem; font-weight: 800; color: var(--acento); }
.plano__nombre { font-size: 0.66rem; letter-spacing: 0.2em; color: var(--tinta-suave); }

/* ── Responsive ────────────────────────────────────────── */
@media (max-width: 900px) {
  .strip { display: none; }
  .cubo { --lado: min(70vw, 44vh, 320px); }
  .cf__sec {
  ...
  .tarjeta--der .tarjeta__linea { margin-left: 0; }
  .tarjeta--der .tarjeta__ctas { justify-content: flex-start; }
  .caption__nombre { font-size: 1.5rem; }
}

/* La escena entera desaparece: quedan la rejilla estática y las tarjetas. */
@media (prefers-reduced-motion: reduce) {
  .cf__escena { display: none; }
  .cf__sec { min-height: auto; padding: 2.5rem var(--hueco); }
  .caption { display: none; }
  .strip__dot { transition: none; }
  .cf__estrellas { transition: none; }
}
</style>
