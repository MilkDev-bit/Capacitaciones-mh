...
<template>
  <div class="page">
    <div class="page-header">
      <div>
        <h2>Mis cursos</h2>
        <p class="subtitle">Crea y gestiona tus cursos de capacitación. Márcalos como públicos para que cualquier usuario se inscriba.</p>
      </div>
      <button class="btn-primary" @click="showForm = !showForm">
        {{ showForm ? 'Cancelar' : '+ Nuevo curso' }}
      </button>
    ...
    <div v-if="showForm" class="card form-card">
      ...
      <div class="form-grid">
        ...
        <div class="field full">
          <label class="toggle-label">
            <input type="checkbox" v-model="form.is_public" />
            <span>Publicar curso (cualquier usuario puede inscribirse)</span>
          </label>
        </div>
      </div>
      <p v-if="error" class="msg error">{{ error }}</p>
    ...
    <div class="table-wrap">
      <table v-if="capacitaciones.length">
        <thead>
          <tr>
            <th>Título</th><th>Tipo</th><th>Visibilidad</th><th>Descripción</th><th>Fecha</th><th></th>
          </tr>
        </thead>
        <tbody>
