...
<template>
  <div class="ia-root" :style="{ '--game-accent': meta.accent, '--game-gradient': meta.gradient }">
        ...
        <div class="ia-completed-card-glass">
          ...
          <div class="completed-content">
            <p class="completed-info-text">
              ¡Excelente trabajo! Tu progreso y tu puntaje ya han sido registrados exitosamente en el sistema.
              <strong>No necesitas repetirlo</strong> para conservar tu avance, pero puedes volver a jugar por práctica
              si lo deseas.
            </p>
    ...
    <div v-if="gameType === '5'" class="game-wrap">
      <div class="game-topbar">
        ...
        <div class="topbar-progress">
          <span class="prog-badge" :style="{ background: meta.gradient }">{{ memoMatched.length }}/{{ memoCards.length /
            2 }}</span>
          <div class="prog-track">
            <div class="prog-fill"
              :style="{ width: `${memoCards.length > 0 ? (memoMatched.length / (memoCards.length / 2)) * 100 : 0}%`, background: meta.gradient }" />
          </div>
        </div>
      </div>
      <div class="memo-grid">
        <button v-for="card in memoCards" :key="card.id" :class="['memo-card',
          ...
          :disabled="memoMatched.includes(card.pairId) || isCompleted" @click="flipCard(card)">
          <div class="card-inner">
            ...
            <div class="card-front" :style="{ background: meta.gradient }">
              ...
              <div class="naipe-center-emblem">
                <div class="glass-emblem-circle">
                  <svg width="38" height="38" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.6"
                    stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="card-back" :style="memoMatched.includes(card.pairId)
              ...
              <div v-if="memoMatched.includes(card.pairId)" class="card-match-mark" :style="{ background: card.color }">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3.5"
                  stroke-linecap="round" stroke-linejoin="round">
              ...
              <span class="card-txt" :style="memoMatched.includes(card.pairId) ? { color: card.color } : {}">{{
                card.text }}</span>
    ...
    <div v-else-if="gameType === '6'" class="game-wrap">
      <p class="game-hint">Arrastra y suelta cada elemento hacia su categoría, o haz clic en un elemento y luego en la
        categoría donde pertenece</p>

      <!-- Pool de items -->
      <div class="drag-pool">
        <button v-for="it in dragItems" :key="it.id" :class="['drag-item',
          selectedDrag === it.id ? 'picked' : '',
          dragAssignments[it.id] ? 'done' : '',
          shakeItems.includes(it.id) ? 'shake' : '']" :disabled="isCompleted" draggable="true"
          @dragstart="handleDragStart($event, it.id)" @click="pickItem(it.id)">
          <span class="drag-item-dot" :style="{
            background: dragAssignments[it.id]
              ? catColor(dragAssignments[it.id])
              : selectedDrag === it.id ? 'var(--game-accent)' : '#cbd5e1'
          }" />
          {{ it.text }}
          <span v-if="dragAssignments[it.id]" class="drag-item-tag"
            :style="{ background: catColor(dragAssignments[it.id]) + '22', color: catColor(dragAssignments[it.id]) }">
      ...
      <!-- Categorías -->
      <div class="drag-cats">
        <div v-for="(cat, ci) in dragCategories" :key="cat"
          :class="['drag-cat', selectedDrag ? 'droppable' : '', dragOverCat === cat ? 'drag-over' : '']"
          :style="{ '--cat-color': CAT_COLORS[ci % CAT_COLORS.length] }" @dragover.prevent="handleDragOver($event, cat)"
          @dragenter.prevent="handleDragOver($event, cat)" @dragleave="handleDragLeave(cat)"
          @drop.prevent="handleDrop($event, cat)" @click="dropInCategory(cat)">
          ...
          <div class="cat-body">
            <div v-for="it in dragItems.filter(i => dragAssignments[i.id] === cat)" :key="it.id" class="cat-chip"
              draggable="true" @dragstart="handleDragStart($event, it.id)" @click.stop="pickItem(it.id)">
              {{ it.text }}
      ...
      <div class="ws-layout">
        ...
        <div class="ws-board-wrap">
          <div class="ws-board" :style="{ userSelect: 'none' }">
            <div v-for="(row, r) in wordGrid" :key="r" class="ws-row">
              <button v-for="(ch, c) in row" :key="c" class="ws-cell" :style="wsCellStyle(r, c)"
                :class="{ 'ws-preview': wsPreview.includes(`${r},${c}`) && !wsFoundCells[`${r},${c}`] }"
                @mousedown.prevent="wsDown(r, c)" @mouseenter="wsEnter(r, c)" @touchstart.prevent="wsDown(r, c)">{{ ch
                }}</button>
            </div>
          </div>
        </div>
        ...
        <div class="ws-panel">
          ...
          <div class="ws-progress-bar">
            <div class="ws-progress-fill"
              :style="{ width: `${(foundWords.length / wordsToFind.length) * 100}%`, background: meta.gradient }" />
          </div>
          <ul class="ws-list">
            <li v-for="(w, wi) in wordsToFind" :key="w" :class="['ws-word', foundWords.includes(w) ? 'found' : '']"
              ...
              <span class="ws-bullet"
                :style="{ background: foundWords.includes(w) ? WS_WORD_COLORS[wi % WS_WORD_COLORS.length] : '#e2e8f0' }" />
              {{ w }}
            </li>
          </ul>
        </div>
      ...
      <div class="fb-list">
        <div v-for="(sent, idx) in fbSentences" :key="idx" :class="['fb-card',
          ...
          <div class="fb-body">
            ...
            <div class="fb-opts">
              <label v-for="opt in sent.options" :key="opt" :class="['fb-opt', fbAnswers[idx] === opt ? 'fb-opt-sel' : '',
                fbChecked && opt === sent.answer ? 'fb-opt-correct' : '',
                fbChecked && fbAnswers[idx] === opt && opt !== sent.answer ? 'fb-opt-wrong' : '']">
                <input type="radio" :name="`fb_${idx}`" :value="opt" v-model="fbAnswers[idx]" class="sr-only"
        ...
        <transition-group name="order-move">
          <div v-for="(it, idx) in orderItems" :key="it._id" :class="['order-card',
            ...
            <div class="order-arrows">
              ...
              <button class="order-arrow" :disabled="idx === orderItems.length - 1 || isCompleted"
    ...
    <div v-else-if="gameType === '11'" class="game-wrap">
        ...
        <div class="hm-visualizer glass-badge-editor">
          <svg viewBox="0 0 200 220" class="hm-svg">
            ...
            <g v-if="hmErrors >= 1" class="hm-part">
              ...
              <!-- Ojos -->
              <text x="122" y="74" font-size="10" fill="#f43f5e" font-weight="bold">{{ hmErrors >= hmMaxErrors ? 'X' :
                '•' }}</text>
              <text x="133" y="74" font-size="10" fill="#f43f5e" font-weight="bold">{{ hmErrors >= hmMaxErrors ? 'X' :
                '•' }}</text>
            </g>

            <!-- 2. Cuerpo Core -->
            <line v-if="hmErrors >= 2" x1="130" y1="88" x2="130" y2="135" stroke="#f43f5e" stroke-width="4"
              stroke-linecap="round" class="hm-part" />

            <!-- 3. Brazo Izquierdo -->
            <line v-if="hmErrors >= 3" x1="130" y1="100" x2="105" y2="120" stroke="#f43f5e" stroke-width="4"
              stroke-linecap="round" class="hm-part" />

            <!-- 4. Brazo Derecho -->
            <line v-if="hmErrors >= 4" x1="130" y1="100" x2="155" y2="120" stroke="#f43f5e" stroke-width="4"
              stroke-linecap="round" class="hm-part" />

            <!-- 5. Pierna Izquierda -->
            <line v-if="hmErrors >= 5" x1="130" y1="135" x2="110" y2="170" stroke="#f43f5e" stroke-width="4"
              stroke-linecap="round" class="hm-part" />

            <!-- 6. Pierna Derecha -->
            <line v-if="hmErrors >= 6" x1="130" y1="135" x2="150" y2="170" stroke="#f43f5e" stroke-width="4"
              stroke-linecap="round" class="hm-part" />
          </svg>
      ...
      <div class="hm-word-container">
        <div v-for="(char, idx) in hmWord.split('')" :key="idx" class="hm-letter-slot"
          :class="{ 'hm-letter-revealed': hmGuessed.includes(char) || isCompleted }">
      ...
      <div class="hm-keyboard">
        <button v-for="char in HM_ALPHABET" :key="char" class="hm-key" :class="{
          'hm-key-ok': hmGuessed.includes(char) && hmWord.includes(char),
          'hm-key-bad': hmGuessed.includes(char) && !hmWord.includes(char)
        }" :disabled="hmGuessed.includes(char) || isCompleted" @click="hmGuessLetter(char)">
          {{ char }}
        </button>
      </div>
    </div>
...
.ia-btn-primary:active:not(:disabled) {
...
.memo-card:not(.matched):hover {
  transform: scale(1.04) translateY(-6px);
}

.memo-card:not(.matched):active {
  transform: scale(0.98);
}

.memo-card.bounce {
  animation: cardBounce 0.6s cubic-bezier(0.36, 0.07, 0.19, 0.97);
}
...
.naipe-corner.top-left {
  top: 16px;
  left: 16px;
}

.naipe-corner.bottom-right {
  bottom: 16px;
  right: 16px;
...
.card-face-corner.top-left {
  top: 12px;
  left: 12px;
}

.card-face-corner.bottom-right {
  bottom: 12px;
  right: 12px;
}
...
.fb-opt-sel .fb-opt-dot {
  background: var(--accent);
}

.fb-opt-correct .fb-opt-dot {
  background: #10b981;
}

.fb-opt-wrong .fb-opt-dot {
  background: #ef4444;
}
...
.order-card.order-ok .order-result-icon {
  color: #10b981;
}

.order-card.order-bad .order-result-icon {
  color: #ef4444;
}
...
<style>
/* ═══════════════════════════════════════════════════════════════════════════
   MODO OSCURO - JUEGOS Y ACTIVIDADES INTERACTIVAS (Global / Unscoped)
═══════════════════════════════════════════════════════════════════════════ */
...
html.dark-theme .game-hint,
html.dark-theme .topbar-hint {
  color: #cbd5e1 !important;
}

html.dark-theme .game-topbar {
...
@media (prefers-color-scheme: dark) {
  html:not(.light-theme) .ia-root {
    background: var(--surface, #1e293b);
    border-color: var(--border-light, rgba(255, 255, 255, 0.12));
  }

  html:not(.light-theme) .ia-title {
    color: #f8fafc;
  }

  html:not(.light-theme) .ia-desc {
    color: #94a3b8;
  }

  html:not(.light-theme) .game-hint,
  html:not(.light-theme) .topbar-hint {
    color: #cbd5e1 !important;
  }

  html:not(.light-theme) .game-topbar {
    background: rgba(15, 23, 42, 0.65) !important;
    border-color: rgba(255, 255, 255, 0.12) !important;
  }

  html:not(.light-theme) .ia-stat {
    background: rgba(30, 41, 59, 0.85);
    border-color: rgba(255, 255, 255, 0.14);
    color: #f8fafc;
  }

  html:not(.light-theme) .ia-stat-pts {
    background: linear-gradient(135deg, rgba(217, 119, 6, 0.35), rgba(180, 83, 9, 0.45));
    border-color: #f59e0b;
    color: #fde68a;
  }

  html:not(.light-theme) .ia-music-btn {
    background: rgba(30, 41, 59, 0.8);
    border-color: rgba(255, 255, 255, 0.15);
    color: #e2e8f0;
  }

  html:not(.light-theme) .ia-music-on {
    background: linear-gradient(135deg, #10b981, #059669);
    color: white;
  }

  html:not(.light-theme) .ia-completed-overlay {
    background: rgba(10, 15, 26, 0.75);
  }

  html:not(.light-theme) .ia-completed-card-glass {
    background: rgba(24, 34, 48, 0.94);
    border-color: rgba(16, 185, 129, 0.5);
    box-shadow: 0 24px 64px rgba(0, 0, 0, 0.65), inset 0 1px 2px rgba(255, 255, 255, 0.12);
  }

  html:not(.light-theme) .completed-topbar {
    border-color: rgba(16, 185, 129, 0.25);
  }

  html:not(.light-theme) .completed-status-badge {
    color: #34d399;
  }

  html:not(.light-theme) .completed-info-text {
    color: #e2e8f0;
  }

  html:not(.light-theme) .completed-info-text strong {
    color: #34d399;
  }

  html:not(.light-theme) .metric-box {
    background: rgba(15, 23, 42, 0.7);
    border-color: rgba(16, 185, 129, 0.3);
  }

  html:not(.light-theme) .metric-title {
    color: #94a3b8;
  }

  html:not(.light-theme) .metric-value {
    color: #f8fafc;
  }

  html:not(.light-theme) .metric-value.status-saved {
    color: #34d399;
  }

  html:not(.light-theme) .btn-completed-replay {
    background: rgba(30, 41, 59, 0.9);
    color: #34d399;
    border-color: rgba(16, 185, 129, 0.45);
  }

  html:not(.light-theme) .btn-completed-replay:hover {
    background: rgba(16, 185, 129, 0.25);
    color: #ffffff;
  }

  html:not(.light-theme) .card-front {
    border-color: rgba(255, 255, 255, 0.25);
  }

  html:not(.light-theme) .card-back {
    background: rgba(30, 41, 59, 0.92);
    border-color: rgba(255, 255, 255, 0.15);
    box-shadow: 0 16px 36px rgba(0, 0, 0, 0.4), inset 0 1px 2px rgba(255, 255, 255, 0.1);
  }

  html:not(.light-theme) .memo-card.matched .card-back {
    background: rgba(24, 32, 47, 0.96);
  }

  html:not(.light-theme) .card-txt {
    color: #f8fafc;
  }

  html:not(.light-theme) .drag-pool {
    background: rgba(15, 23, 42, 0.55);
    border-color: rgba(255, 255, 255, 0.16);
  }

  html:not(.light-theme) .drag-item {
    background: rgba(30, 41, 59, 0.88);
    border-color: rgba(255, 255, 255, 0.18);
    color: #f8fafc;
  }

  html:not(.light-theme) .drag-item.picked {
    background: color-mix(in srgb, var(--accent) 30%, rgba(30, 41, 59, 0.95));
    color: white;
  }

  html:not(.light-theme) .drag-cat {
    background: color-mix(in srgb, var(--cat-color) 15%, rgba(15, 23, 42, 0.78));
    border-color: color-mix(in srgb, var(--cat-color) 50%, rgba(255, 255, 255, 0.18));
  }

  html:not(.light-theme) .drag-cat.drag-over {
    background: color-mix(in srgb, var(--cat-color) 25%, rgba(15, 23, 42, 0.95)) !important;
  }

  html:not(.light-theme) .cat-name {
    color: #f8fafc;
  }

  html:not(.light-theme) .cat-count {
    background: color-mix(in srgb, var(--cat-color) 25%, rgba(15, 23, 42, 0.9));
    color: var(--cat-color);
  }

  html:not(.light-theme) .cat-chip {
    background: rgba(30, 41, 59, 0.9);
    border-color: color-mix(in srgb, var(--cat-color) 45%, rgba(255, 255, 255, 0.15));
    color: #f8fafc;
  }

  html:not(.light-theme) .cat-empty {
    background: color-mix(in srgb, var(--cat-color) 5%, rgba(15, 23, 42, 0.5));
    border-color: color-mix(in srgb, var(--cat-color) 35%, rgba(255, 255, 255, 0.18));
    color: #94a3b8;
  }

  html:not(.light-theme) .ws-board {
    background: rgba(15, 23, 42, 0.65);
    border-color: rgba(255, 255, 255, 0.16);
  }

  html:not(.light-theme) .ws-cell {
    background: rgba(30, 41, 59, 0.85);
    border-color: rgba(255, 255, 255, 0.14);
    color: #f8fafc;
  }

  html:not(.light-theme) .ws-cell:hover {
    background: rgba(99, 102, 241, 0.25);
    border-color: #a5b4fc;
  }

  html:not(.light-theme) .ws-panel {
    background: rgba(15, 23, 42, 0.65);
    border-color: rgba(255, 255, 255, 0.16);
  }

  html:not(.light-theme) .ws-panel-title {
    color: #f8fafc;
  }

  html:not(.light-theme) .ws-word {
    color: #cbd5e1;
  }

  html:not(.light-theme) .fb-card {
    background: rgba(15, 23, 42, 0.65);
    border-color: rgba(255, 255, 255, 0.16);
  }

  html:not(.light-theme) .fb-card.fb-correct {
    background: rgba(16, 185, 129, 0.15);
    border-color: #10b981;
  }

  html:not(.light-theme) .fb-card.fb-wrong {
    background: rgba(239, 68, 68, 0.15);
    border-color: #ef4444;
  }

  html:not(.light-theme) .fb-text {
    color: #f8fafc;
  }

  html:not(.light-theme) .fb-opt {
    background: rgba(30, 41, 59, 0.85);
    border-color: rgba(255, 255, 255, 0.18);
    color: #f8fafc;
  }

  html:not(.light-theme) .fb-opt-sel {
    background: color-mix(in srgb, var(--accent) 30%, rgba(30, 41, 59, 0.95));
    color: #a5b4fc;
  }

  html:not(.light-theme) .fb-opt-correct {
    background: rgba(16, 185, 129, 0.22) !important;
    color: #34d399 !important;
  }

  html:not(.light-theme) .fb-opt-wrong {
    background: rgba(239, 68, 68, 0.22) !important;
    color: #f87171 !important;
  }

  html:not(.light-theme) .order-card {
    background: rgba(15, 23, 42, 0.65);
    border-color: rgba(255, 255, 255, 0.16);
  }

  html:not(.light-theme) .order-card.order-ok {
    background: rgba(16, 185, 129, 0.15);
    border-color: #10b981;
  }

  html:not(.light-theme) .order-card.order-bad {
    background: rgba(239, 68, 68, 0.15);
    border-color: #ef4444;
  }

  html:not(.light-theme) .order-txt {
    color: #f8fafc;
  }

  html:not(.light-theme) .order-arrow {
    background: rgba(30, 41, 59, 0.85);
    border-color: rgba(255, 255, 255, 0.18);
    color: #cbd5e1;
  }

  html:not(.light-theme) .hm-visualizer {
    background: rgba(15, 23, 42, 0.55);
    border-color: rgba(255, 255, 255, 0.16);
  }

  html:not(.light-theme) .hm-hint-box {
    background: rgba(244, 63, 94, 0.12);
    border-color: rgba(244, 63, 94, 0.35);
  }

  html:not(.light-theme) .hm-hint-text {
    color: #f8fafc;
  }

  html:not(.light-theme) .hm-lives-label {
    color: #e2e8f0;
  }

  html:not(.light-theme) .hm-letter-slot {
    background: rgba(30, 41, 59, 0.85);
    border-color: rgba(255, 255, 255, 0.18);
    color: #f8fafc;
  }

  html:not(.light-theme) .hm-letter-revealed {
    background: linear-gradient(135deg, rgba(16, 185, 129, 0.25), rgba(5, 150, 105, 0.35));
    color: #34d399;
    border-color: #10b981;
  }

  html:not(.light-theme) .hm-key {
    background: rgba(30, 41, 59, 0.85);
    border-color: rgba(255, 255, 255, 0.18);
    color: #f8fafc;
  }
