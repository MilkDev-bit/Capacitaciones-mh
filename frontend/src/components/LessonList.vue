  ...
  <div class="lesson-list">
    ...
    <div v-else>
      <div v-for="(l, index) in lessons" :key="l.id" class="lesson-item">
        <div v-if="editMode === l.id" class="lesson-edit-pane slide-down-enter-active">
          ...
          <div class="field mt-3">
            ...
            <input type="number" v-model="editForm.duracion_min" class="field-input" style="width:100px" />
          </div>
        ...
        <div v-else class="lesson-row">
          <div class="lesson-drag">
            <button class="icon-btn" :disabled="index === 0" @click="move(index, -1)">
              <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M5 15l7-7 7 7"/></svg>
            </button>
            <button class="icon-btn" :disabled="index === lessons.length - 1" @click="move(index, 1)">
      ...
      <div v-if="showCreate" class="lesson-create-pane mt-4 slide-down-enter-active">
        ...
        <div class="field mt-3">
          <label>Duración (minutos)</label>
          <input type="number" v-model="createForm.duracion_min" class="field-input" style="width:100px" />
        </div>
        <div class="edit-actions mt-4">
...
.lesson-item { background: var(--surface); border: 1px solid var(--border-light); border-radius: var(--r-md); overflow: hidden; }
.lesson-row { display: flex; align-items: center; padding: 12px; gap: 12px; }
.lesson-drag { display: flex; flex-direction: column; gap: 2px; }
.icon-btn { background: none; border: none; color: var(--muted); cursor: pointer; padding: 4px; border-radius: 4px; display: flex; align-items: center; justify-content: center; transition: all 0.2s; }
.icon-btn:hover:not(:disabled) { background: var(--surface-soft); color: var(--dark); }
.icon-btn:disabled { opacity: 0.3; cursor: not-allowed; }
.icon-btn.text-danger:hover { color: var(--danger); background: var(--danger-bg); }

.lesson-info { flex: 1; display: flex; flex-direction: column; min-width: 0; }
.lesson-title { font-weight: 600; font-size: 0.95rem; color: var(--dark); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.lesson-meta { font-size: 0.75rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.05em; }

.lesson-actions { display: flex; gap: 4px; }

.lesson-edit-pane, .lesson-create-pane { padding: 20px; background: var(--surface-soft); border-top: 1px solid var(--border-light); border-radius: var(--r-md); }
.lesson-edit-pane h4, .lesson-create-pane h4 { margin: 0 0 16px 0; font-size: 1.1rem; }
...
.btn-dashed { width: 100%; padding: 16px; border: 2px dashed var(--border); background: transparent; color: var(--muted); border-radius: var(--r-md); cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; font-weight: 600; transition: all 0.2s; }
.btn-dashed:hover { border-color: var(--brand); color: var(--brand); background: var(--brand-light); }
</style>
