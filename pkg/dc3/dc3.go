...
// Ocupaciones es el catálogo de ÁREAS Y SUBÁREAS OCUPACIONALES del Catálogo
// Nacional de Ocupaciones.
//
// Transcrito del REVERSO de la plantilla embebida. Son 66 claves y no deben
// confundirse con AreasTematicas: el reverso del formato lleva DOS catálogos
// distintos y cada uno alimenta una casilla distinta.
//
//	Ocupación específica    -> este catálogo (01, 01.1, 05.1, 11.3 ...)
//	Área temática del curso -> AreasTematicas (1000 ... 9000)
//
// La casilla pide la CLAVE, no el nombre del puesto. Se emitieron constancias
// con "SUPERVISOR" escrito a mano ahí: eso es lo que el trabajador hace, no una
// ocupación del catálogo, y con ello el documento no es válido.
//
// Debe coincidir con el catálogo del mismo nombre en frontend/src/utils/dc3.ts.
var Ocupaciones = map[string]string{
...
// OcupacionValida comprueba que la clave pertenezca al Catálogo Nacional de
// Ocupaciones.
//
// Se valida en el servidor por lo mismo que AreaValida: la clave va impresa en
// un documento legal y una petición directa podría colar cualquier cadena.
func OcupacionValida(clave string) bool {
	_, ok := Ocupaciones[strings.TrimSpace(clave)]
	return ok
}
